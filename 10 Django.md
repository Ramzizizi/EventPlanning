...
- F обрабатывает совершает обработку над полем модели без выгрузки поля в память
...
- вложение атомарной операции внутрь другой автомарной операции позволяет верхнюю транзакцию вне зависимости от вложенной, а фиксация фложенной не вызовет фиксации верхней 
...
- При получении запроса происходит перенаправление на одну из заданных views соотвуствующих полученному url


- преобразователи path
- str для строк, int для чисел, uuid для UUID, path для путей (производит анализ строки на url), slug - промежуточая строка


- обработчики ошибок в urlpath
- по умолчанию присуствуют обработки ошибок 400, 403, 404 и 500. Можно добавить кастомные ошибки или использовать кастомные страницы для установленных ошибок
...
- определяет показ трейсбек ошибок и другие данные
